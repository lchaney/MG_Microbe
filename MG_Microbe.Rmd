...
```{r, Sum_GE, echo = FALSE}
   ...
   cld(glht(fit4, linfct = mcp(Treatment = "Tukey"))) #Tukey groups
```
...
```{r, LeafSum34_GE, echo = FALSE}
      ...
      cld(glht(fit7, linfct = mcp(Treatment = "Tukey"))) #Tukey groups
```
...
```{r, HeightRGRC_GE, echo = FALSE}
      anova(fit37)
         ...
         cld(glht(fit37, linfct = mcp(Treatment = "Tukey"))) #Tukey groups
``` 
...
```{r,sg surface treat, echo = FALSE}
    grid.arrange(arrangeGrob(FSsurface.antibiotic, FSsurface.inoculum, FSsurface.field, FSsurface.autoclave, nrow = 2))
```
